function [u1_step, u2_step, t, uv,z] = control_update(dt, time, td, to, ls, gs, ps, control_state, last_bounce_time, plot, control_num, leg_angle_desired,u0);
    %control state vars
%     in_air = 1;
...
%         dd_ds_leg_angle, dd_des_leg_angle];
    bcs = [gs, ge, d_ds_leg_angle, d_des_leg_angle, ...
        dd_ds_leg_angle, dd_des_leg_angle];

    %tf = last_bounce_time; %Can sub in the equation for this 
    if control_state == 1;
    t0 = time; tf = t0+last_bounce_time;
    gs
    ge
    [u1_step,u2_step,z, t] = bvp_fun( [bcs,t0,tf, dt], ls );
    ...
    bcs = [ls, le, gs, ge, d_ds_leg_length, d_des_leg_length,...
        ...
        dd_ds_leg_angle, dd_des_leg_angle];
    t0 = time; tf = t0+last_bounce_time;
    [u1_step,u2_step,z, t] = bvp_fun_ground( [bcs,t0,tf, dt] );
    uv{1} = u1_step; uv{2} = u2_step;
    t = t + t0;
    diffv = abs(time-t); [val,tind] = min(diffv); 
    u1_step = u1_step(1); u2_step = u2_step(1);
